...
const Login2fa: React.FC<Props> = ({ setIsAuth, isAuth, setUserId, token2fa }) => {
  ...
  const generateSecret = async () => {
    try {
      const url2Fa = 'https://literate-space-garbanzo-vjvjp6xjpvvfp57j-5000.app.github.dev/pong/users/auth/2fa/' + id;
  ...
  const handle2fa = async () => {
    ...
    try {
      const validate = await axios.post('https://literate-space-garbanzo-vjvjp6xjpvvfp57j-5000.app.github.dev/pong/users/auth/2fa', { token: otp, userId: id + '' });
